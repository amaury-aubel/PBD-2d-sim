#ifndef Magnum_Examples_PBDSimulation2D_SDFObject_h
#define Magnum_Examples_PBDSimulation2D_SDFObject_h
/*
...
struct SDFObject {
    ...
    explicit SDFObject(): center{0, 0}, radii{1}, type{ObjectType::Circle}, negativeInside{true}, orient(0.0f) {}
    ...
    explicit SDFObject(const Vector2& center_, const Vector2& radii_, ObjectType type_, bool negativeInside_ = true):
        ...
        negativeInside{negativeInside_}, orient{1.0f}
    {
    ...
    void setOrient(Float rad) { orient = rad; }
    ...
    Float signedDistance(const Vector2& pos) const {
        switch(type) {
            ...
            case ObjectType::Box: {
                const Float s = std::sin(-orient);
                const Float c = std::cos(-orient);
                Vector2 rot_pos(c * pos[0] - s * pos[1], s * pos[0] + c * pos[1]);
                const Float dx = Math::abs(rot_pos[0] - center[0]) - radii[0];
                const Float dy = Math::abs(rot_pos[1] - center[1]) - radii[1];
                Float dist { 0 };
                if(dx < 0 && dy < 0) {
                    dist = Math::max(dx, dy);
                } else {
